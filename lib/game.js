...
async function syncSchedule(gameId, opts = {}) {
  ...
  const tx = db.transaction(() => {
    for (const af of apiFixtures) {
      ...
      // Match an existing local fixture: group fixtures by team-pair (they were
      // generated without an ext_id), knockout fixtures strictly by ext_id (they
      // always carry the provider id). Matching knockouts by pair would risk
      // overwriting a group fixture that shares the same two teams.
      const existing = isGroup
        ? fixtureByPair.get(pairKey(home, away))
        : (extId ? fixtureByExt.get(extId) : null);
...
function getParticipantState(token) {
  ...
  return { me, game, myTeams, myFixtures, table, bracket: knockoutBracket(me.game_id) };
}

// Knockout rounds in bracket order (must match STAGE_MAP's display values).
const KO_ROUND_ORDER = ['Round of 32', 'Round of 16', 'Quarter-final', 'Semi-final', 'Third place', 'Final'];

/**
 * Knockout fixtures grouped into bracket rounds (column-per-round). Each match
 * carries both teams with their owning participant. Returns [] before any
 * knockout fixtures exist.
 */
function knockoutBracket(gameId) {
  const fixtures = listFixtures(gameId).filter((f) => f.stage && f.stage !== 'group');
  if (!fixtures.length) return [];
  const owners = ownerMap(gameId);
  const side = (name, goals) => {
    const info = owners.get(name) || {};
    return { team: name || null, ownerName: info.ownerName ?? null, goals };
  };
  ...
  const orderIdx = (s) => { const i = KO_ROUND_ORDER.indexOf(s); return i === -1 ? 99 : i; };
  ...
  return [...byStage.keys()]
    ...
    .map((stage) => ({
      ...
      matches: byStage.get(stage)
        ...
        .map((f) => ({
          id: f.id, status: f.status, winner: f.winner,
          home: side(f.home_team, f.home_goals),
          away: side(f.away_team, f.away_goals),
        })),
    }));
}
...
function getAdminState(gameId) {
  ...
  return { game, participants, byParticipant, teams, fixtures, owners, table, bracket: knockoutBracket(gameId) };
}
...
module.exports = {
  createGame, getGame, listGames, deleteGame,
  ...
  getParticipantByToken, getParticipantState, getAdminState, knockoutBracket,
};
